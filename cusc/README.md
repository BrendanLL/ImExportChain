...
If you haven't already clone the repository to a directory of your choice, and change to the `cusc` directory

```
git clone https://github.com/hyperledger/fabric-samples.git
cd fabric-samples/cusc
```

This `README.md` file is in the the `cusc` directory, the source code for client applications and the contracts ins in the `ogranization` directory, and some helper scripts are in the `roles` directory.
...
`cusc/organization/magnetocorp/application` directory or the `cusc/organization/magnetocorp/application-java`
...
node addToWallet.js
# or 
java -cp target/cusc-0.0.1-SNAPSHOT.jar org.magnetocorp.AddToWallet
```
...
node issue.js
# or 
java -cp target/cusc-0.0.1-SNAPSHOT.jar org.magnetocorp.Issue
```
...
`fabric-samples/cusc/` directory

`./roles/digibank.sh` 

You can now run the applications to buy and redeem the paper. Change to either the 
`cusc/organization/digibank/application` directory or  `cusc/organization/digibank/application-java`
...
node addToWallet.js
# or 
java -cp target/cusc-0.0.1-SNAPSHOT.jar org.digibank.AddToWallet
```
...
node buy.js
# or
java -cp target/cusc-0.0.1-SNAPSHOT.jar org.digibank.Buy
```
...
node redeem.js
# or 
java -cp target/cusc-0.0.1-SNAPSHOT.jar org.digibank.Redeem
```
